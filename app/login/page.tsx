...
import { ShieldCheck, Loader2, AlertCircle, Sparkles, KeyRound } from "lucide-react"
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background text-foreground">
      ...
      <main className="flex-1 flex items-center justify-center py-16 px-4">
        {/* Auth card */}
        <div className="w-full max-w-md rounded-2xl border border-border bg-card/15 p-6 sm:p-8 backdrop-blur-md space-y-6 relative overflow-hidden">
          <div className="absolute -top-12 -left-12 w-36 h-36 bg-primary/10 rounded-full blur-[70px] pointer-events-none" />

          {/* Heading */}
          <div className="text-center space-y-2">
            <h1 className="text-2xl font-bold text-white flex items-center justify-center gap-2">
              <KeyRound className="size-5 text-primary" /> DigitalServices<span className="text-primary">4U</span>
            </h1>
          ...
          {/* Tabs */}
          <div className="grid grid-cols-2 bg-muted/40 p-1.5 rounded-xl border border-border/50">
            <button
              onClick={() => {
                setActiveTab("login")
                setError(null)
              ...
              className={`py-2 rounded-lg text-xs font-semibold tracking-wide cursor-pointer transition-colors ${
                activeTab === "login"
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <button
              ...
              className={buttonVariants({
                className: "w-full justify-center gap-2 cursor-pointer font-medium h-10 select-none mt-2",
              })}
            >
              {loading ? (
                <>
          ...
          {/* Bottom Trust */}
          <div className="flex items-center justify-center gap-2 text-[10px] text-muted-foreground border-t border-border/50 pt-4">
